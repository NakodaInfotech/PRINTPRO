...
Public Class OrderFilter
    ...
    Private Sub OrderFilter_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        Try
    ...
    Private Sub cmdshow_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdshow.Click
        Try
            ...
            OBJORDER.WHERECLAUSE = " {ORDERMASTER.ORDER_cmpid}=" & CmpId & " and {ORDERMASTER.ORDER_yearid}=" & YearId
            ...
            If CMBNAME.Text.Trim <> "" Then OBJORDER.WHERECLAUSE = OBJORDER.WHERECLAUSE & " and {LEDGERS.ACC_CMPNAME}='" & CMBNAME.Text.Trim & "'"
            If CMBITEMNAME.Text.Trim <> "" Then OBJORDER.WHERECLAUSE = OBJORDER.WHERECLAUSE & " and {ITEMMASTER.ITEM_name}='" & CMBITEMNAME.Text.Trim & "'"
            If CMBITEMCODE.Text.Trim <> "" Then OBJORDER.WHERECLAUSE = OBJORDER.WHERECLAUSE & " and {ITEMMASTER.ITEM_CODE}='" & CMBITEMCODE.Text.Trim & "'"

            If CMBFOLDED.Text.Trim = "Folded" Then
                OBJORDER.WHERECLAUSE = OBJORDER.WHERECLAUSE & " and {ITEMMASTER.ITEM_FOLDED}=True"
            ElseIf CMBFOLDED.Text.Trim = "Unfolded" Then
                OBJORDER.WHERECLAUSE = OBJORDER.WHERECLAUSE & " and {ITEMMASTER.ITEM_FOLDED}=False"
            End If


            If RDDETAILS.Checked = True Then
                If CHKSUMMARY.CheckState = CheckState.Unchecked Then OBJORDER.FRMSTRING = "ORDERDTLS" Else OBJORDER.FRMSTRING = "ORDERSUMM"
            ...
            ElseIf RBPENDING.Checked = True Then
                OBJORDER.FRMSTRING = "PENDINGORDERS"
                OBJORDER.WHERECLAUSE = OBJORDER.WHERECLAUSE & " AND {ORDERMASTER_DESC.ORDER_QTY} - {ORDERMASTER_DESC.ORDER_OUTQTY} > 0 AND {ORDERMASTER_DESC.ORDER_CLOSE}= False"

            ElseIf RBORDERNOTDESPATCHED.Checked = True Then
                OBJORDER.FRMSTRING = "ORDERNOTDESPATCHED"
                OBJORDER.WHERECLAUSE = " {ORDERNOTDESPATCHED.YEARID}=" & YearId
                If CMBNAME.Text.Trim <> "" Then OBJORDER.WHERECLAUSE = OBJORDER.WHERECLAUSE & " and {ORDERNOTDESPATCHED.NAME}='" & CMBNAME.Text.Trim & "'"
                If CMBITEMNAME.Text.Trim <> "" Then OBJORDER.WHERECLAUSE = OBJORDER.WHERECLAUSE & " and {ORDERNOTDESPATCHED.ITEMNAME}='" & CMBITEMNAME.Text.Trim & "'"
                If CMBITEMCODE.Text.Trim <> "" Then OBJORDER.WHERECLAUSE = OBJORDER.WHERECLAUSE & " and {ORDERNOTDESPATCHED.ITEMCODE}='" & CMBITEMCODE.Text.Trim & "'"
                OBJORDER.Show()
                Exit Sub

            ElseIf RBCLOSED.Checked = True Then
                OBJORDER.FRMSTRING = "CLOSEDORDERS"
                OBJORDER.WHERECLAUSE = OBJORDER.WHERECLAUSE & " AND {ORDERMASTER_DESC.ORDER_CLOSE}= True "

            ElseIf RBCOMPLETED.Checked = True Then
                OBJORDER.FRMSTRING = "COMPLETEORDERS"
                OBJORDER.WHERECLAUSE = OBJORDER.WHERECLAUSE & " AND {ORDERMASTER_DESC.ORDER_QTY} - {ORDERMASTER_DESC.ORDER_OUTQTY} <= 0 AND {ORDERMASTER_DESC.ORDER_CLOSE}= False"
