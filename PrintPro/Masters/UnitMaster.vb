    ...
    Private Sub txtname_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtname.Validating
        Try
            ...
            If (edit = False) Or (edit = True And LCase(Tempname) <> LCase(txtname.Text.Trim)) Then
                dt = objclscommon.search("unit_name", "", "unitMaster", " and unit_name = '" & txtname.Text.Trim & "' And Unit_yearid = " & YearId & " And Unit_cmpid = " & CmpId)
                If dt.Rows.Count > 0 Then
                    MsgBox("Unit Name Already Exists", MsgBoxStyle.Critical, "PRINTPRO")
                    e.Cancel = True
                End If
            End If
            pcase(txtname)
        ...
        Try
            If txtabbr.Text.Trim <> "" Then
                ...
                If (edit = False) Or (edit = True And LCase(Tempabbr) <> LCase(txtabbr.Text.Trim)) Then
                    dt = objclscommon.search("unit_abbr", "", "unitMaster", " and unit_abbr = '" & txtabbr.Text.Trim & "' And Unit_yearid = " & YearId & " And Unit_cmpid = " & CmpId)
                    If dt.Rows.Count > 0 Then
                        MsgBox("Unit Abbr Already Exists", MsgBoxStyle.Critical, "ACCTRADE")
                        e.Cancel = True
                    End If
                End If
                uppercase(txtname)
