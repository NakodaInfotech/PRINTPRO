...
Public Class PurchaseReturnDetails
    ...
    Sub fillgrid()
        Try
            ...
            Dim dt As DataTable = objclsCMST.search("  PURCHASERETURN.PR_no AS SRNO, PURCHASERETURN.PR_date AS DATE, LEDGERS.Acc_cmpname AS NAME, ISNULL(LEDGERS.ACC_GSTIN, '') AS GSTIN, ISNULL(PURCHASERETURN.PR_BILLNO, '') AS BILLNO, PURCHASERETURN.PR_BILLDATE AS BILLDATE, ISNULL(REGISTERMASTER.register_name, '') AS REGNAME, ISNULL(PURCHASERETURN.PR_PARTYBILL, '') AS PARTYBILL, PURCHASERETURN.PR_PARTYDATE AS PARTYDATE, ISNULL(PURCHASERETURN.PR_EWAYBILLNO, '') AS EWAYBILLNO, PURCHASERETURN.PR_TOTALQTY AS TOTALQTY, ISNULL(PURCHASERETURN.PR_remarks, '') AS REMARKS, ISNULL(PURCHASERETURN.PR_BILLAMT, 0) AS AMT, ISNULL(PURCHASERETURN.PR_TOTALCGSTAMT, 0) AS CGSTAMT, ISNULL(PURCHASERETURN.PR_TOTALSGSTAMT, 0) AS SGSTAMT, ISNULL(PURCHASERETURN.PR_SGSTPER, 0) AS SGSTPER, ISNULL(PURCHASERETURN.PR_CGSTPER, 0) AS CGSTPER, ISNULL(PURCHASERETURN.PR_IGSTPER, 0) AS IGSTPER, ISNULL(PURCHASERETURN.PR_TOTALIGSTAMT, 0) AS IGSTAMT, ISNULL(PURCHASERETURN.PR_SUBTOTAL, 0) AS SUBTOTAL, ISNULL(PURCHASERETURN.PR_GRANDTOTAL, 0) AS GRANDTOTAL, ISNULL(AGENTLEDGERS.Acc_cmpname, '') AS AGENT, ISNULL(PURCHASERETURN.PR_PARTYREFNO, '') AS PARTYREFNO, ISNULL(CREDITLEDGERS.Acc_cmpname, '') AS CREDITNAME, ISNULL(PURCHASERETURN.PR_NOOFBALES, 0) AS NOOFBALES, ISNULL(DELIVERYATLEDGERS.Acc_cmpname, '') AS DELIVERYAT, ISNULL(TOCITYMASTER.city_name, '') AS TOCITY , ISNULL(USERMASTER.User_Name, '') AS CREATEDBY, ISNULL(DISPATCHLEDGERS.Acc_cmpname, '') AS DISPATCHFROM, ISNULL(CITYMASTER.city_name, '') AS FROMCITY, ISNULL(PURCHASERETURN.PR_VEHICLENO, '') AS VEHICLENO   ", "", "PURCHASERETURN INNER JOIN LEDGERS ON PURCHASERETURN.PR_ledgerid = LEDGERS.Acc_id LEFT OUTER JOIN CITYMASTER ON PURCHASERETURN.PR_FROMCITYID = CITYMASTER.city_id LEFT OUTER JOIN LEDGERS AS DISPATCHLEDGERS ON PURCHASERETURN.PR_DISPATCHFROMID = DISPATCHLEDGERS.Acc_id LEFT OUTER JOIN USERMASTER ON PURCHASERETURN.PR_userid = USERMASTER.User_id LEFT OUTER JOIN LEDGERS AS AGENTLEDGERS ON PURCHASERETURN.PR_AGENTID = AGENTLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS DELIVERYATLEDGERS ON PURCHASERETURN.PR_DELIVERYATID = DELIVERYATLEDGERS.Acc_id LEFT OUTER JOIN CITYMASTER AS TOCITYMASTER ON PURCHASERETURN.PR_TOCITYID = TOCITYMASTER.city_id LEFT OUTER JOIN REGISTERMASTER ON PURCHASERETURN.PR_PURREGID = REGISTERMASTER.register_id LEFT OUTER JOIN LEDGERS AS CREDITLEDGERS ON PURCHASERETURN.PR_CREDITLEDGERID = CREDITLEDGERS.Acc_id ", " AND PURCHASERETURN.PR_YEARID = " & YearId)
