...
Public Class PurchaseReturn
        ...
        Try
            ...
            If GRIDPURRET.RowCount > 0 Then
                ...
                For Each row As DataGridViewRow In GRIDPURRET.Rows
                    If row.Cells(GUNIT.Index).EditedFormattedValue <> "" Then
            ...
            If EDIT = True Then
                ...
                If dt.Rows.Count > 0 Then
                    For Each dr As DataRow In dt.Rows
                        ...
                        'Item Grid
                        GRIDPURRET.Rows.Add(dr("GRIDSRNO").ToString, dr("ITEM").ToString, dr("HSNCODE").ToString, dr("SIZE").ToString, dr("WT"), dr("QTY").ToString, dr("UNIT").ToString, dr("RATE").ToString, dr("AMT").ToString, dr("GRNNO"), dr("GRNSRNO"), dr("TYPE"), dr("DONE"))
    ...
    Sub fillcmb()
        Try
            ...
            If CMBDELIVERYAT.Text.Trim = "" Then fillname(CMBDELIVERYAT, EDIT, " AND GROUPMASTER.GROUP_SECONDARY = 'SUNDRY CREDITORS'  AND LEDGERS.ACC_TYPE = 'ACCOUNTS'")
            If CMBITEM.Text.Trim = "" Then FILLNONINVITEM(CMBITEM, EDIT)
...
LINE1:
    ...
    Sub GETHSNCODE()
        Try
            ...
            If Convert.ToDateTime(ACTUALINVDATE.Text).Date >= "01/07/2017" And ITEMNAME <> "" Then
                ...
                Dim DT As DataTable = OBJCMN.search("  ISNULL(HSNMASTER.HSN_CODE, '') AS HSNCODE, ISNULL(HSNMASTER.HSN_CGST, 0) AS CGSTPER, ISNULL(HSNMASTER.HSN_SGST, 0) AS SGSTPER, ISNULL(HSNMASTER.HSN_IGST, 0) AS IGSTPER ", "", "HSNMASTER INNER JOIN NONINVITEMMASTER ON HSNMASTER.HSN_ID = NONINVITEMMASTER.NONINV_HSNCODEID AND HSNMASTER.HSN_YEARID = NONINVITEMMASTER.NONINV_yearid ", " AND NONINVITEMMASTER.NONINV_NAME= '" & CMBITEM.Text.Trim & "' AND HSNMASTER.HSN_CMPID='" & CmpId & "' AND HSNMASTER.HSN_YEARID='" & YearId & "' ORDER BY HSNMASTER.HSN_ID DESC")
                If DT.Rows.Count > 0 Then
    ...
    Private Sub CMBQTYUNIT_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles CMBUNIT.Validating
        Try
    ...
    Sub fillgrid()
        ...
        If GRIDDOUBLECLICK = False Then
            GRIDPURRET.Rows.Add(Val(TXTSRNO.Text.Trim), CMBITEM.Text.Trim, TXTHSNCODE.Text.Trim, TXTSIZE.Text.Trim, TXTWT.Text.Trim, Format(Val(TXTQTY.Text.Trim), "0.00"), CMBUNIT.Text.Trim, Format(Val(TXTRATE.Text.Trim), "0.00"), Format(Val(TXTAMT.Text.Trim), "0.00"), 0, 0, "", 0)
            getsrno(GRIDPURRET)
        ElseIf GRIDDOUBLECLICK = True Then
            ...
            GRIDPURRET.Item(GSIZE.Index, TEMPROW).Value = TXTSIZE.Text.Trim
            GRIDPURRET.Item(GWT.Index, TEMPROW).Value = TXTWT.Text.Trim
            GRIDPURRET.Item(gQty.Index, TEMPROW).Value = TXTQTY.Text.Trim
            GRIDPURRET.Item(GUNIT.Index, TEMPROW).Value = CMBUNIT.Text.Trim
            GRIDPURRET.Item(GRATE.Index, TEMPROW).Value = Format(Val(TXTRATE.Text.Trim), "0.00")
            GRIDPURRET.Item(GAMT.Index, TEMPROW).Value = TXTAMT.Text.Trim
    ...
    Private Sub TXTAMT_Validated(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TXTAMT.Validated
        If CMBITEM.Text.Trim <> "" And Val(TXTQTY.Text.Trim) > 0 And Val(TXTRATE.Text.Trim) > 0 Then
    ...
    Sub EDITROW()
        Try
            ...
            If GRIDPURRET.CurrentRow.Index >= 0 And GRIDPURRET.Item(gsrno.Index, GRIDPURRET.CurrentRow.Index).Value <> Nothing Then
                ...
                TXTSIZE.Text = GRIDPURRET.Item(GSIZE.Index, GRIDPURRET.CurrentRow.Index).Value.ToString
                TXTWT.Text = GRIDPURRET.Item(GWT.Index, GRIDPURRET.CurrentRow.Index).Value.ToString
                TXTQTY.Text = GRIDPURRET.Item(gQty.Index, GRIDPURRET.CurrentRow.Index).Value.ToString
                CMBUNIT.Text = GRIDPURRET.Item(GUNIT.Index, GRIDPURRET.CurrentRow.Index).Value.ToString
    ...
    Private Sub TXTRATE_Validated(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TXTRATE.Validated, TXTQTY.Validated
        TOTAL()
    End Sub
    ...
    Private Sub TXTBARCODE_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        '        Try
    ...
    Private Sub CMDSELECTINVOICE_Click(sender As Object, e As EventArgs) Handles CMDSELECTINVOICE.Click
        Try
            If CMBNAME.Text.Trim = "" Then
                MsgBox("Please Select Party First", MsgBoxStyle.Critical)
                CMBNAME.Focus()
    ...
    Private Sub CMDSELECTCHALLAN_Click(sender As Object, e As EventArgs) Handles CMDSELECTCHALLAN.Click
        Try
            ...
            If DTTABLE.Rows.Count > 0 Then
                ...
                Dim DT1 As DataTable = OBJCMN.search(" PURCHASERETURNCHALLAN.PRCH_NO AS PRCHNO, ISNULL(NONINVITEMMASTER.NONINV_NAME,'') AS ITEM ,ISNULL(HSNMASTER.HSN_CODE,'') AS HSNCODE ,ISNULL(HSN_CGST,0) AS CGSTPER, ISNULL(HSN_SGST,0) AS SGSTPER, ISNULL(HSN_IGST,0) AS IGSTPER, SUM(ISNULL(PURCHASERETURNCHALLAN_DESC.PRCH_QTY, 0)) AS QTY, ISNULL(PURCHASERETURNCHALLAN_DESC.PRCH_SIZE, '') AS SIZE ,  ISNULL(PURCHASERETURNCHALLAN_DESC.PRCH_WT, 0) AS WT , ISNULL(UNITMASTER.unit_abbr, '') AS UNIT , ISNULL(PURCHASERETURNCHALLAN_DESC.PRCH_RATE, 0) AS RATE  ", "", " PURCHASERETURNCHALLAN INNER JOIN PURCHASERETURNCHALLAN_DESC ON PURCHASERETURNCHALLAN.PRCH_NO = PURCHASERETURNCHALLAN_DESC.PRCH_NO AND  PURCHASERETURNCHALLAN.PRCH_YEARID = PURCHASERETURNCHALLAN_DESC.PRCH_YEARID LEFT OUTER JOIN NONINVITEMMASTER ON PURCHASERETURNCHALLAN_DESC.PRCH_ITEMID = NONINVITEMMASTER.NONINV_ID LEFT OUTER JOIN UNITMASTER ON PURCHASERETURNCHALLAN_DESC.PRCH_UNITID = UNITMASTER.unit_id LEFT OUTER JOIN HSNMASTER ON NONINVITEMMASTER.NONINV_HSNCODEID = HSNMASTER.HSN_ID ", " AND PURCHASERETURNCHALLAN.PRCH_NO IN(" & TXTPRCHNO.Text.Trim & ")  and PURCHASERETURNCHALLAN.PRCH_YEARID = " & YearId & " GROUP BY PURCHASERETURNCHALLAN.PRCH_NO, ISNULL(NONINVITEMMASTER.NONINV_NAME,''),  ISNULL(HSNMASTER.HSN_CODE,''), ISNULL(HSNMASTER.HSN_CGST, 0), ISNULL(HSNMASTER.HSN_SGST, 0), ISNULL(HSNMASTER.HSN_IGST, 0), ISNULL(PURCHASERETURNCHALLAN_DESC.PRCH_SIZE, '') ,  ISNULL(PURCHASERETURNCHALLAN_DESC.PRCH_WT, 0) ,  ISNULL(UNITMASTER.unit_abbr, '') , ISNULL(PURCHASERETURNCHALLAN_DESC.PRCH_RATE, 0)  order by PURCHASERETURNCHALLAN.PRCH_NO ")
                If DT1.Rows.Count > 0 Then
                    For Each dr As DataRow In DT1.Rows
                        ...
                        GRIDPURRET.Rows.Add(0, dr("ITEM"), dr("HSNCODE"), dr("SIZE"), dr("WT"), Val(dr("QTY")), dr("UNIT"), dr("RATE"), 0, dr("PRCHNO"), 0, "", 0)
                        ...
                        If dr("ITEM").ToString <> "" And Convert.ToDateTime(PRDATE.Text).Date >= "01/07/2017" Then
                            ...
                            Dim DTHSN As DataTable = OBJCMN.search("  ISNULL(HSNMASTER.HSN_CODE, '') AS HSNCODE, ISNULL(HSNMASTER.HSN_CGST, 0) AS CGSTPER, ISNULL(HSNMASTER.HSN_SGST, 0) AS SGSTPER, ISNULL(HSNMASTER.HSN_IGST, 0) AS IGSTPER ", "", "HSNMASTER INNER JOIN NONINVITEMMASTER ON HSNMASTER.HSN_ID = NONINVITEMMASTER.NONINV_HSNCODEID AND HSNMASTER.HSN_YEARID = NONINVITEMMASTER.NONINV_yearid ", " AND  NONINVITEMMASTER.NONINV_NAME= '" & dr("ITEM") & "'  AND HSNMASTER.HSN_CMPID='" & CmpId & "' AND HSNMASTER.HSN_YEARID='" & YearId & "' ORDER BY HSNMASTER.HSN_ID DESC")
    ...
    Private Sub CMBDISPATCHFROM_Validated(sender As Object, e As EventArgs) Handles CMBDISPATCHFROM.Validated
        Try
            If CMBNAME.Text.Trim <> CMBDISPATCHFROM.Text.Trim Then
                Dim OBJCMN As New ClsCommon
                Dim DT As DataTable = OBJCMN.search("ISNULL(LEDGERS.ACC_KMS,0) AS KMS, ISNULL(CITYMASTER.CITY_NAME,'') AS FROMCITY ", "", " LEDGERS LEFT OUTER JOIN CITYMASTER ON LEDGERS.Acc_cityid = CITYMASTER.city_id ", " and LEDGERS.acc_cmpname = '" & CMBDISPATCHFROM.Text.Trim & "' AND LEDGERS.acc_YEARid = " & YearId)
                If DT.Rows.Count > 0 Then
