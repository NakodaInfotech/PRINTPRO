    ...
    Sub fillgrid(Optional ByVal where As String = "")
        Try
            ...
            If FRMSTRING = "PURRETURN" Then
                DT1 = objclspreq.search(" DISTINCT CAST(0 AS BIT) AS CHK, PURCHASERETURNCHALLAN.PRCH_NO AS PRCHNO, PURCHASERETURNCHALLAN.PRCH_DATE AS DATE, ISNULL(PURCHASERETURNCHALLAN.PRCH_CHALLANNO, '') AS PARTYCHALLANNO, LEDGERS.Acc_cmpname AS NAME, SUM(ISNULL(PURCHASERETURNCHALLAN_DESC.PRCH_QTY, 0)) AS QTY, ISNULL(PURCHASERETURNCHALLAN_DESC.PRCH_SIZE, '') AS SIZE, ISNULL(PURCHASERETURNCHALLAN_DESC.PRCH_WT, 0) AS WT, ISNULL(GODOWNMASTER.GODOWN_name, '') AS GODOWN, ISNULL(CAST(STATEMASTER.state_remark AS VARCHAR), '') AS STATECODE, ISNULL(LEDGERS.ACC_GSTIN, '') AS GSTIN, ISNULL(PURCHASERETURNCHALLAN.PRCH_NOOFBALES, 0) AS NOOFBALES, ISNULL(TRANSLEDGERS.Acc_cmpname, '') AS TRANSNAME, ISNULL(PURCHASERETURNCHALLAN.PRCH_LRNO, '') AS LRNO, ISNULL(PURCHASERETURNCHALLAN.PRCH_LRDATE, '') AS LRDATE, ISNULL(NONINVITEMMASTER.NONINV_NAME, '') AS ITEM  ", "", "  PURCHASERETURNCHALLAN INNER JOIN PURCHASERETURNCHALLAN_DESC ON PURCHASERETURNCHALLAN.PRCH_NO = PURCHASERETURNCHALLAN_DESC.PRCH_NO AND PURCHASERETURNCHALLAN.PRCH_YEARID = PURCHASERETURNCHALLAN_DESC.PRCH_YEARID LEFT OUTER JOIN NONINVITEMMASTER ON PURCHASERETURNCHALLAN_DESC.PRCH_ITEMID = NONINVITEMMASTER.NONINV_ID LEFT OUTER JOIN LEDGERS AS TRANSLEDGERS ON PURCHASERETURNCHALLAN.PRCH_TRANSID = TRANSLEDGERS.Acc_id LEFT OUTER JOIN GODOWNMASTER ON PURCHASERETURNCHALLAN.PRCH_GODOWNID = GODOWNMASTER.GODOWN_id LEFT OUTER JOIN LEDGERS ON PURCHASERETURNCHALLAN.PRCH_LEDGERID = LEDGERS.Acc_id LEFT OUTER JOIN STATEMASTER ON TRANSLEDGERS.Acc_stateid = STATEMASTER.state_id ", " AND PURCHASERETURNCHALLAN.PRCH_DONE = 0 AND PURCHASERETURNCHALLAN.PRCH_YEARID = " & YearId & where & " GROUP BY PURCHASERETURNCHALLAN.PRCH_NO, PURCHASERETURNCHALLAN.PRCH_DATE, ISNULL(PURCHASERETURNCHALLAN.PRCH_CHALLANNO, ''), LEDGERS.Acc_cmpname, ISNULL(NONINVITEMMASTER.NONINV_NAME, ''), ISNULL(PURCHASERETURNCHALLAN_DESC.PRCH_SIZE, ''), ISNULL(PURCHASERETURNCHALLAN_DESC.PRCH_WT, 0), ISNULL(GODOWNMASTER.GODOWN_name, ''), ISNULL(CAST(STATEMASTER.state_remark AS VARCHAR), ''), ISNULL(LEDGERS.ACC_GSTIN, ''), ISNULL(PURCHASERETURNCHALLAN.PRCH_NOOFBALES, 0), ISNULL(TRANSLEDGERS.Acc_cmpname, ''), ISNULL(PURCHASERETURNCHALLAN.PRCH_LRNO, ''), ISNULL(PURCHASERETURNCHALLAN.PRCH_LRDATE, ''), ISNULL(NONINVITEMMASTER.NONINV_NAME, '') order by PURCHASERETURNCHALLAN.PRCH_NO ")
            Else
    ...
    Private Sub cmdok_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdok.Click
        Try
            If FRMSTRING = "PURRETURN" Then
                ...
                DT1.Columns.Add("PRCHNO")
                DT1.Columns.Add("DATE")
                DT1.Columns.Add("PARTYCHALLANNO")
                ...
                For i As Integer = 0 To gridbill.RowCount - 1
                    ...
                    If Convert.ToBoolean(dtrow("CHK")) = True Then
                        DT1.Rows.Add(dtrow("PRCHNO"), dtrow("DATE"), dtrow("PARTYCHALLANNO"), dtrow("NAME"), dtrow("ITEM"), Val(dtrow("QTY")), dtrow("SIZE"), Val(dtrow("WT")), dtrow("GODOWN"), dtrow("GSTIN"), dtrow("STATECODE"), dtrow("NOOFBALES"), dtrow("TRANSNAME"), dtrow("LRNO"), dtrow("LRDATE"))
                    End If
                Next
                Me.Close()
            Else
                ...
                For i As Integer = 0 To gridbill.RowCount - 1
                    Dim dtrow As DataRow = gridbill.GetDataRow(i)
                    If Convert.ToBoolean(dtrow("CHK")) = True Then
                        DT1.Rows.Add(dtrow("SRCHNO"), dtrow("DATE"), dtrow("PARTYCHALLANNO"), dtrow("NAME"), dtrow("ITEM"), Val(dtrow("QTY")), dtrow("SIZE"), Val(dtrow("WT")), dtrow("GODOWN"), dtrow("GSTIN"), dtrow("STATECODE"), dtrow("NOOFBALES"), dtrow("TRANSNAME"), dtrow("LRNO"), dtrow("LRDATE"))
                    End If
                Next
                Me.Close()
