...
Public Class Receipt
    ...
    Sub fillgridINVOICE()
        ...
        Dim DT As DataTable = objpayment.GETBILLS(CmpId, cmbname.Text.Trim, Locationid, YearId, Convert.ToDateTime(ACCDATE.Text).Date)
        If DT.Rows.Count > 0 Then
            SETGRIDINVOICE(DT)
    ...
    Private Sub cmbbillno_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles cmbbillno.Validating
        Try
            ...
            If cmbbillno.Text.Trim <> "" Then
                ...
                Next
                If Not BLN Then
                    MsgBox("Invalid Bill No.", MsgBoxStyle.Critical, "PRINTPRO")
                    cmbbillno.Focus()
                    Exit Sub
                End If
        ...
        Try
            ...
            If txtsrno.Text.Trim.Length > 0 And Val(txtamt.Text) > 0 Then
                If cmbpaytype.Text = "Against Bill" And cmbbillno.Text.Trim = "" Then
                    MsgBox("Select Bill First", MsgBoxStyle.Critical, "PRINTPRO")
                    cmbpaytype.Focus()
                    Exit Sub
                End If
                ...
                If cmbbillno.Text.Trim <> "" Then
                    For Each ROW As DataGridViewRow In gridpayment.Rows
                        If (ROW.Cells(gbillno.Index).Value = cmbbillno.Text.Trim And GRIDDOUBLECLICK = False) Or (GRIDDOUBLECLICK = True And ROW.Cells(gbillno.Index).Value = cmbbillno.Text.Trim And ROW.Index <> TEMPROW) Then
                            MsgBox("Bill Already present in Grid below", MsgBoxStyle.Critical, "PRINTPRO")
                            cmbpaytype.Focus()
                            Exit Sub
                        End If
                    Next
        ...
        Try
            ...
            Else
                If EDIT = True Then
                    ...
                    Dim tempmsg As Integer = MsgBox("Delete Receipt Entry Permanently?", MsgBoxStyle.YesNo, "PRINTPRO")
                    If tempmsg = vbYes Then
                ...
                If txtchqno.Text.Trim <> "" And txtchqno.Text.Trim <> "RTGS" And txtchqno.Text.Trim <> "NEFT" And txtchqno.Text.Trim <> "IMPS" And txtchqno.Text.Trim <> "ONLINE" Then
                    ...
                    If DT.Rows.Count > 0 Then
                        If (EDIT = False) Or (EDIT = True And CHQNO <> txtchqno.Text.Trim) Then
                            MsgBox("Chq. No. Already Present with this Bank in Receipt No." & DT.Rows(0).Item(0), MsgBoxStyle.Critical, "PRINTPRO")
                            e.Cancel = True
                        End If
                    End If
                End If
...
1:
    ...
    Private Sub TOOLWHATSAPP_Click(sender As Object, e As EventArgs) Handles TOOLWHATSAPP.Click
        Try
            Dim DT As New DataTable
            Dim OBJCMN As New ClsCommon
            If EDIT = True Then SENDWHATSAPP(TEMPRECEIPTNO)
            DT = OBJCMN.Execute_Any_String("UPDATE RECEIPTMASTER SET RECEIPT_SENDWHATSAPP = 1 WHERE RECEIPT_NO = " & TEMPRECEIPTNO & " AND RECEIPT_YEARID = " & YearId, "", "")
    ...
    Async Sub SENDWHATSAPP(RECNO As Integer)
        Try
            ...
            OBJWHATSAPP.AGENTNAME = ""
            OBJWHATSAPP.PATH.Add(Application.StartupPath & " \ RECEIPT_" & Val(RECNO) & ".pdf")
            OBJWHATSAPP.FILENAME.Add("RECEIPT_" & Val(RECNO) & ".pdf")
