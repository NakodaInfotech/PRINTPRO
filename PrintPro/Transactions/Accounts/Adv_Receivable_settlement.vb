...
Public Class Adv_Receivable_settlement
    ...
    Sub fillgrid()
        Try
            ...
            If flag_adv_settlement = True Then
                dt = objclsCMST.search("  PAYMENTDATE as Date,PAYINITIALS as [Bill No.], PAYMENTAMT as [Amount], PAYTYPE as [Pay Type], ACCNAME as [Account], PAYMENTNO as [No], REGISTERID as [Reg Id], REGNAME as [Reg Name], NAME ", "", "  dbo.payment_report4paytype  ", "  and payment_report4paytype.CMPID = " & CmpId & " and payment_report4paytype.LOCATIONID= " & Locationid & " and payment_report4paytype.YEARID= " & YearId & condition)
            ElseIf flag_Rec_settlement = True Then
                dt = objclsCMST.search(" RECEIPTDATE as Date, PAYINITIALS as [Bill No.], RECEIPTAMT as [Amount], PAYTYPE as [Pay Type], ACCNAME as [Account], RECEIPTNO as [No], REGISTERID as [Reg Id], REGNAME as [Reg Name], NAME ", "", "  dbo.receipt_report4paytype  ", "  and receipt_report4paytype.CMPID = " & CmpId & " and receipt_report4paytype.LOCATIONID= " & Locationid & " and receipt_report4paytype.YEARID= " & YearId & condition)
            End If

            griddetails.DataSource = dt

            'griddetail.Columns(0).Width = 90
            'griddetails.Columns(1).Width = 90
            'griddetails.Columns(2).Width = 100
            'gridAdvRec.Columns(3).Width = 150
            'griddetails.Columns(4).Width = 250
            'gridAdvRec.Columns(5).Width = 80
            'gridAdvRec.Columns(6).Width = 80
            'gridAdvRec.Columns(7).Width = 80

            'gridAdvRec.Columns(5).Visible = False
            'gridAdvRec.Columns(6).Visible = False
            'gridAdvRec.Columns(7).Visible = False
