...
Public Class CFormEntry
    ...
    Private Sub cmddelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmddelete.Click
        Try
            ...
            If MsgBox("Delete Form Entry?", MsgBoxStyle.YesNo) = MsgBoxResult.No Then Exit Sub
            Dim dtrow As DataRow = gridbill.GetFocusedDataRow
            Dim alParaval As New ArrayList
            If dtrow("FORMNO") <> "" Then
    ...
    Private Sub CMDADD_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CMDADD.Click
        Try
            If RBPENDING.Checked = False Then Exit Sub
            ...
            If CMBNAME.Text.Trim <> "" Then WHERECLAUSE = WHERECLAUSE & " NAME = '" & CMBNAME.Text.Trim & "'"
            Dim DT As DataTable = OBJCMN.search(" TYPE,  CFORMVIEW.BILL, REGTYPE, CFORMVIEW.BILLINITIALS, CFORMVIEW.DATE AS BILLDATE, NAME, CFORMVIEW.AMT, CITYNAME, STATENAME, FORMNO, FORMDATE, RECDATE, REMARKS ", "", " CFORMVIEW ", WHERECLAUSE & " AND BILL = " & Val(TXTBILLNO.Text.Trim) & " AND FORMNO = '' AND FORMNAME = '" & CMBFORM.Text.Trim & "' AND CFORMVIEW.YEARID = " & YearId & " AND CFORMVIEW.TYPE = '" & CMBTYPE.Text.Trim & "' ORDER BY CFORMVIEW.BILL ")
            If DT.Rows.Count > 0 Then
                If IsNothing(gridbill.DataSource) Then
                    gridbilldetails.DataSource = DT
                Else
                    Dim NEWDT As DataTable = gridbilldetails.DataSource
                    Dim DTROW As DataRow = DT.Rows(0)
                    Dim CHECKDT() As DataRow = NEWDT.Select("BILLINITIALS = '" & DTROW("BILLINITIALS") & "'")
                    If CHECKDT.Length <= 0 Then NEWDT.ImportRow(DTROW)
                End If
            End If
            TXTBILLNO.Clear()
            TXTBILLNO.Focus()
