...
Public Class SaleRegister
    ...
    Sub total()
        Try
            ...
            For Each row As DataGridViewRow In gridregister.Rows
                ...
                row.Cells(GNETT.Index).Value = Val(row.Cells(GCREDIT.Index).Value) - Val(row.Cells(GDEBIT.Index).Value) - Val(row.Cells(GTAX.Index).Value)
                TXTST.Text = Format((Val(TXTST.Text.Trim) + Val(row.Cells(GTAX.Index).Value)), "0.00")
    ...
    Private Sub PrintToolStripButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles PrintToolStripButton.Click
        Try
            ...
            Dim DT As DataTable = OBJCMN.Execute_Any_String("DELETE FROM TEMPMONTHLYREGISTER WHERE YEARID = " & YearId, "", "")
            For Each ROW As DataGridViewRow In gridregister.Rows
                DT = OBJCMN.Execute_Any_String("INSERT INTO TEMPMONTHLYREGISTER VALUES (" & Val(ROW.Index) & ",'" & ROW.Cells(GMONTH.Index).Value & "'," & Val(ROW.Cells(GDEBIT.Index).Value) & "," & Val(ROW.Cells(GCREDIT.Index).Value) & "," & Val(ROW.Cells(GRUNNINGBAL.Index).Value) & "," & CmpId & "," & YearId & ")", "", "")
            Next
