            ...
            If e.RowIndex >= 0 Then
                With GRIDBILL.Rows(e.RowIndex).Cells(GRIDBILL.Columns("INVCHK").Index)
                    ...
                    Else
                        ...
                        Dim OBJCMN As New ClsCommon
                        Dim DT As DataTable = OBJCMN.search("*", "", " (SELECT INVOICEMASTER.INVOICE_TOTALTAXABLEAMT AS TAXABLEAMT, ISNULL(INVOICE_PRINTINITIALS,'') AS INVPRINTINITIALS FROM INVOICEMASTER WHERE INVOICEMASTER.INVOICE_INITIALS = '" & GRIDBILL.Rows(e.RowIndex).Cells(GRIDBILL.Columns("INVBILLINITIALS").Index).Value & "' AND INVOICEMASTER.INVOICE_YEARID = " & YearId & " UNION ALL SELECT OPENINGBILL.BILL_AMT AS TAXABLEAMT, ISNULL(BILL_PRINTINITIALS,'') AS INVPRINTINITIALS FROM  OPENINGBILL WHERE OPENINGBILL.BILL_INITIALS = '" & GRIDBILL.Rows(e.RowIndex).Cells(GRIDBILL.Columns("INVBILLINITIALS").Index).Value & "' AND OPENINGBILL.BILL_YEARID = " & YearId & ") AS T", "")
                        If DT.Rows.Count > 0 Then
...
LINE1:
    ...
    Private Sub TOOLWHATSAPP_Click(sender As Object, e As EventArgs) Handles TOOLWHATSAPP.Click
        Try
            ...
            DT = OBJCMN.Execute_Any_String("UPDATE CREDITNOTEMASTER SET CN_SENDWHATSAPP = 1 WHERE CN_NO = " & TEMPCNNO & " AND CN_YEARID = " & YearId, "", "")
