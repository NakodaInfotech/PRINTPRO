...
Public Class OpeningClosingStock
    ...
    Private Sub cmdok_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdok.Click
        Try
            ...
            If Val(TXTCLOSING.Text.Trim) > 0 Then DT = OBJCMN.Execute_Any_String(" INSERT INTO ACCMASTER VALUES (" & Val(FROMLEDGERID) & "," & Val(TXTCLOSING.Text.Trim) & "," & TOLEDGERID & ",0,'" & Format(AccTo.Date, "MM/dd/yyyy") & "','STOCK','',''," & CmpId & ",0," & Userid & "," & YearId & ",0,GETDATE())", "", "")
            ...
            If Val(TXTOPENING.Text.Trim) > 0 Then DT = OBJCMN.Execute_Any_String(" INSERT INTO ACCMASTER VALUES (" & Val(FROMLEDGERID) & "," & Val(TXTOPENING.Text.Trim) & "," & TOLEDGERID & ",0,'" & Format(AccFrom.Date, "MM/dd/yyyy") & "','OPENING','',''," & CmpId & ",0," & Userid & "," & YearId & ",0,GETDATE())", "", "")
    ...
    Private Sub OpeningClosingStock_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            Dim OBJCMN As New ClsCommon
            Dim DT As DataTable = OBJCMN.search("OPENINGSTOCK, CLOSINGSTOCK", "", " OPENINGCLOSINGSTOCK ", " AND YEARID = " & YearId)
            If DT.Rows.Count > 0 Then
                TXTOPENING.Text = Val(DT.Rows(0).Item("OPENINGSTOCK"))
                TXTCLOSING.Text = Val(DT.Rows(0).Item("CLOSINGSTOCK"))
            End If
        Catch ex As Exception
            Throw ex
        End Try
