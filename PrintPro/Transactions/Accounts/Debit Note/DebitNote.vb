            ...
            If e.RowIndex >= 0 Then
                With GRIDBILL.Rows(e.RowIndex).Cells(GRIDBILL.Columns("INVCHK").Index)
                    ...
                    Else
                        ...
                        Dim OBJCMN As New ClsCommon
                        Dim DT As DataTable = OBJCMN.search("*", "", "(SELECT ISNULL(PURCHASEMASTER.BILL_TOTALTAXABLEAMT, 0) AS TOTALTAXABLEAMT FROM PURCHASEMASTER WHERE PURCHASEMASTER.BILL_INITIALS = '" & GRIDBILL.Rows(e.RowIndex).Cells(GRIDBILL.Columns("INVBILLINITIALS").Index).Value & "' AND PURCHASEMASTER.BILL_YEARID = " & YearId & " UNION ALL SELECT INVOICEMASTER.INVOICE_TOTALTAXABLEAMT AS TOTALTAXABLEAMT FROM INVOICEMASTER WHERE INVOICEMASTER.INVOICE_INITIALS = '" & GRIDBILL.Rows(e.RowIndex).Cells(GRIDBILL.Columns("INVBILLINITIALS").Index).Value & "' AND INVOICEMASTER.INVOICE_YEARID = " & YearId & " UNION ALL SELECT ISNULL(NONPURCHASE.NP_TOTALTAXABLEAMT, 0) AS TOTALTAXABLEAMT FROM NONPURCHASE WHERE NONPURCHASE.NP_INITIALS = '" & GRIDBILL.Rows(e.RowIndex).Cells(GRIDBILL.Columns("INVBILLINITIALS").Index).Value & "' AND NONPURCHASE.NP_YEARID = " & YearId & " UNION ALL SELECT ISNULL(BILL_AMT, 0) AS TOTALTAXABLEAMT FROM OPENINGBILL WHERE OPENINGBILL.BILL_INITIALS = '" & GRIDBILL.Rows(e.RowIndex).Cells(GRIDBILL.Columns("INVBILLINITIALS").Index).Value & "' AND OPENINGBILL.BILL_YEARID = " & YearId & ") AS T", "")
                        If DT.Rows.Count > 0 Then TXTACTUALINVAMT.Text = Val(DT.Rows(0).Item("TOTALTAXABLEAMT"))
